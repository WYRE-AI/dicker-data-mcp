...
function startHttpServer(): void {
  ...
  const httpServer = createHttpServer(async (req, res) => {
    ...
    if (url.pathname === '/health') {
      ...
      // mode, credentials arrive per-request via X-DickerData-* headers, not
      // at startup - checking getCredentials() here would 503 the container
    ...
    const accountCode = isGatewayMode ? (req.headers['x-dickerdata-account-code'] as string | undefined) : undefined;
    const accessToken = isGatewayMode ? (req.headers['x-dickerdata-access-token'] as string | undefined) : undefined;
    ...
    if (accountCode && accessToken) {
      await runWithCredentials({ accountCode, accessToken }, handle);
    } else {
      await handle();
    }
  });
